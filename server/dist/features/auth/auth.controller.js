...
import { getUserByEmail, getUserById, getUserPasswordById, getUserEmailById, createUser, updateUser, deleteUser, } from './auth.dal.js';
import { storeRefreshToken, deleteRefreshToken, } from './token.dal.js';
import { RegisterSchema, LoginSchema, UpdatePasswordSchema, PasswordSchema, } from '@flashlearn/schema-db';
import { genAuthToken, genRefreshToken, verifyToken } from '../../services/token-service.js';
...
export const logout = asyncHandler(async (req, res) => {
    const token = verifyToken(req.cookies.flashLearn_refreshToken, process.env.REFRESH_TOKEN_SECRET || '');
...
export const refreshToken = asyncHandler(async (req, res) => {
    ...
    const verifiedToken = verifyToken(refreshToken, process.env.REFRESH_TOKEN_SECRET || '');
    if (!verifiedToken) {
        res.status(401);
