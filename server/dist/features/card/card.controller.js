...
export const viewCards = asyncHandler(async (req, res) => {
    const userId = req.userId;
    ...
    const card = await getCardsBySetIdWithPagination(setId, page, userId);
    if (card) {
        throw new AppError({ message: 'user is not authorized to view this set', status: 400 });
    }
    res.status(200).json({
...
export const putDeleteCard = asyncHandler(async (req, res) => {
    const userId = req.userId;
    const cardId = req.params.cardId;
